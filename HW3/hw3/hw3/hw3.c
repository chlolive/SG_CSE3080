...
void make_minheap(element* E, int* E_num, nodePtr graph[], int vertex_num, int** visit);
...
int main() {
    ...
    */
    ...
    FILE* outFp = fopen("D://hw3_result.txt", "w");
    ...
    for (i = 0; i < edge_num; i++) parent[i] = -1;
    
    //visit array: if visit[i][j]=1, it means the path vertex i to vertex j has been passed
    int** visit = (int**)malloc(sizeof(int*) * vertex_num);
    for (i = 0; i < vertex_num; i++) visit[i] = (int*)malloc(sizeof(int) * vertex_num);
    ...
    //Store the information of undirected graph
    nodePtr input; 
    ...
    for (int x = 0; x < vertex_num; x++) {
        printf("graph[%d] : ", x);
        for (nodePtr nodeWalker = graph[x]; nodeWalker != NULL; nodeWalker = nodeWalker->link) {
            printf("%d(%d) ", nodeWalker->dest, nodeWalker->weight);
        }
        printf("\n");
    }
    ...
    make_minheap(E,&E_num, graph, vertex_num, visit);
    for (int x = 1; x <= E_num; x++) {
        printf("%d(%d, %d) ", E[x].weight, E[x].from, E[x].to);
    }
    printf("\n");
    ...
    free(MST);
    free(parent);
    for (i = 0; i < vertex_num; i++) free(visit[i]);
    free(visit);
...
void make_minheap(element* E, int* E_num, nodePtr graph[], int vertex_num, int **visit) {
    ...
    for (j = 0; j < vertex_num; j++) {
        for (nodeWalker = graph[j]; nodeWalker != NULL; nodeWalker = nodeWalker->link) {
            ...
            //Insert the node into min heap only if the edge has not been visited
            if (visit[j][nodeWalker->dest] == 0 && visit[nodeWalker->dest][j]==0) {
                E[i].from = j;
                E[i].to = nodeWalker->dest;
                E[i].weight = nodeWalker->weight;
                
                printf("E[%d] = from %d -> to %d, weight %d\n", i, E[i].from, E[i].to, E[i].weight);
                
                
                ++(*E_num);
                
                visit[j][nodeWalker->dest] = 1;
                visit[nodeWalker->dest][j] = 1;
